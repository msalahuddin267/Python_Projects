...
def see_my_profie(request):# fro gennerell user
...
def change_my_password(request):# in this case will just  allow user to change thrir  change password with old password for simplysity 
...
def apply_as_editors(request):
   if request.method=='POST':
     ...
     if form.is_valid():
        ...
        return redirect('see_my_profie')
     else:messages.success(request, 'something wrong !!!!!')

   else:form = EditorApplyForm()
